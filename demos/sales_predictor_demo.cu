...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        Linear l1(NUM_FEATURES, 64);
        Linear l2(64, 32);
        Linear l3(32, 1);

        Sequential model({&l1, new Relu(), &l2, new Sigmoid(), &l3});
        ...
        // Fully automated parameter registration from the Sequential container
        for (Parameter &p : model.parameters()) {
            optimizer.add_parameter(*p.weight, *p.grad);
        }
        ...
        for (int e = 0; e < EPOCHS; e++) {
            ...
            cudaMemcpy(gpu_index_ptr, cpu_indices.data(), cpu_indices.size() * sizeof(int),
